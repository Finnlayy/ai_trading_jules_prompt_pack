...
## 2026-05-31 - [Security] Prevent Unauthenticated Webhooks with Live Brokers
**Learning:** Configurations shouldn't silently default to insecure states when critical features (like live trading) are active.
**Action:** When adding secrets, add startup validation that fails fast (`ValueError`) if the secret is missing but dependent critical features are enabled. Provide a fallback `UserWarning` if it's safe to run without the secret (e.g. simulation).
